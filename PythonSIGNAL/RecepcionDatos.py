...
class ControladorSerial:
    """Maneja la conexión y lectura de datos por puerto serial en segundo plano."""
    def __init__(self):
        self.conexion = None
        self.hilo_activo = False
        self.ultimo_dato = None
        self.lock = threading.Lock() # Evita colisiones entre los hilos al leer/escribir datos

    def conectar(self, puerto, baudrate):
        try:
            self.conexion = serial.Serial(puerto, baudrate, timeout=1)
            time.sleep(1) # Tiempo para estabilizar el puerto
            self.hilo_activo = True
            
            # Iniciamos el hilo de lectura continua
    ...
    def _bucle_lectura(self):
            """Filosofía sencilla de lectura sincronizada."""
            while self.hilo_activo:
                if self.conexion and self.conexion.is_open:
                    try:
                        # 1. Buscamos el byte de sincronización 'S' (en bytes es b'S')
                        if self.conexion.read(1) == b' ':
                            # 2. Si lo encontramos, leemos con seguridad los 2 bytes del ADC
                            data = self.conexion.read(2) 
                            
                            if len(data) == 2:
                                valor_adc = int.from_bytes(data, byteorder='little')
                                
                                # Guardamos el dato de forma segura
                                with self.lock:
                                    # Conversión opcional si quieres pasar de ADC (0-1023) a °C aquí:
                                    # Ejemplo para LM35 en Arduino UNO (5V / 1024 pasos * 100)
                                    # self.ultimo_dato = (valor_adc * 5.0 / 1024.0) * 100.0
                                    
                                    self.ultimo_dato = (float(valor_adc) * (5 / 1023) * 100) - 3.5
                    except Exception as e:
                        pass # Ignoramos errores de lectura temporales
                time.sleep(0.01)

    def obtener_ultimo_dato(self):
        """El Front-end llama a esta función para obtener el valor más reciente."""
        with self.lock:
...
# =====================================================================
# FRONT-END: INTERFAZ GRÁFICA (Solo visualización y control de usuario)
# =====================================================================
class InterfazControl:
    def __init__(self, root):
        ...
        # --- Instancia del Back-end ---
        self.backend = ControladorSerial()

        # --- Variables de estado del Front-end ---
        ...
        self.med_temp = None # <--- INICIA EN NONE PARA NO MOSTRAR UN 25 FALSO
        self.med_speed = 0.0
        ...
        # Bucle de actualización visual en tiempo real
    ...
    def iniciar(self):
        ...
        try:
            # Delegamos la conexión al Back-end
            self.backend.conectar(self.puerto_com.get(), self.baudrate)
            self.backend.limpiar_datos() # Limpiamos basura anterior
            self.med_temp = None
    ...
    def parar(self):
        ...
        self.pwm_temp = 0.0
        self.pwm_motor = 0.0
        self.backend.desconectar() # Desconectamos desde el Back-end
    ...
    def actualizar_interfaz(self):
        """Bucle Visual: Lee el backend y actualiza la GUI."""
        if self.ejecutando:
            self.contador_tiempo += 1
            
            # 1. Obtener el dato más reciente del Backend Serial
            ...
            if self.sistema_activo.get() == "Temperatura":
                ...
                # Solo calcula PID de temperatura si ya recibimos datos reales
                if self.med_temp is not None:
                    error_t = target_t - self.med_temp
            ...
            # Guardar en historiales (Usamos 0.0 temporalmente si no hay temp para no quebrar la gráfica)
        ...
        # Programa la siguiente actualización del Front-end
